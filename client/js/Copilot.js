$(document).ready(function() {
	...
	var currentPieceColors = new Array(4);
	var critical;
	var banner;
	var testRotate = new Array(4);
	//currently drawn board
	var board = new Array(200);
	...
	var respawnable = new Boolean();
	var fallspeed = 1;	
	
	alive = true;
	respawnable = false;
	//initialize colors
	for(var x=0;x<4;x++)currentPieceColors[i]=1;
	//initialize board
	for(var x=0;x<10;x++)
	{
		localBoard[x]==new Array(21);
	}
	for (var i = 0; i < 11; i++) {
		...
		.attr({move: {left: false, right: false, rotate: false, drop: false}})
			...
			.bind("KeyDown", function(e) {
				...
				if(this.move.right) 
					if(checkBounds(1)){
						if(checkCollision(1)){
							for(var i=0;i<4;i++)currentPiece[i].x +=30;
							this.move.right = false;
						}
					}
				if(this.move.left)
				...
				if(this.move.drop) {
					if(checkBounds(4)){
						if(checkCollision(0))for(var i=0;i<4;i++)currentPiece[i].y+=10;							
					}
				}
				if(checkBounds(5)){
					if(checkCollision(0))for(var i=0;i<4;i++)currentPiece[i].y+=fallspeed;
				}
			});			
			var pieceChoice = Math.floor((Math.random()*7)+1)
			for(var i=0;i<4;i++)currentPieceColors[i] = Math.floor((Math.random()*4)+1)
	...
	function checkCollision(a){
		...
		if((currentPiece[0]._y/30) <=0 || (currentPiece[1]._y/30) <=0 ||
			(currentPiece[2]._y/30) <=0 || (currentPiece[3]._y/30) <=0)return true;
		if(a==1)
			return(
	...
	function checkBounds(a){
		...
		if(a==4)return (currentPiece[0]._y + 40 && currentPiece[1]._y + 40 && 
			currentPiece[2]._y + 40 && currentPiece[3]._y + 40) < 600;
		if(a==5)return (currentPiece[0]._y + 30 && currentPiece[1]._y + 30 && 
	...
	function placeBlocks(){
		...
		deathcounter++;
		if(deathcounter%100==0)console.log(deathcounter);
		if(alive==true){
			if (deathcounter==600) fallspeed=2;
			if (deathcounter==1200) fallspeed=3;
			if (deathcounter==3600) fallspeed=4;
			if (deathcounter==6000) fallspeed=5;
		}
		...
		var a=0;
		if(currentPiece[0]==null ||currentPiece[1]==null ||currentPiece[2]==null ||currentPiece[3]==null)return;
		//localboard's x, y or the bottom of page
		if((currentPiece[0]._y + 30)  >= 600|| (currentPiece[1]._y + 30) >= 600||
			(currentPiece[2]._y + 30)  >= 600|| (currentPiece[3]._y + 30) >= 600){a=1}//continue
		//if any currentPiece[i]'s x, y+30 are equal to a 
			...
			localBoard[Math.floor(currentPiece[1]._x/30)][Math.ceil(currentPiece[1]._y/30)] != 0 ||
			localBoard[Math.floor(currentPiece[2]._x/30)][Math.ceil(currentPiece[2]._y/30)] != 0 ||
			localBoard[Math.floor(currentPiece[3]._x/30)][Math.ceil(currentPiece[3]._y/30)] != 0){a=1}
		//if piece is above the board
		if((currentPiece[0]._y/30) <=0 || (currentPiece[1]._y/30) <=0 ||
			(currentPiece[2]._y/30) <=0 || (currentPiece[3]._y/30) <=0){a=0};
		if(a==0)return;
		
		//begin code for placement & things related to placing
		var rowsToCheck = new Array();
		for(var i=0;i<4;i++){
		...
		for(var i=0;i<4;i++)checkRow(rowsToCheck[i]);
		destroyPiece();
		if(!checkLose()){
			redrawBoard();
			createNewPiece();
	...
	function checkRow(a){
		...
		if((count1+count2+count3+count4)==10){
			...
			for (var i = 0; i < 10; i++)localBoard[i][a]=0;
